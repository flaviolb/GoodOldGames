...
StartMulti:
	RLLog.Debug("StartMulti - Started")
	;-----------------------------------------------------------------------------------------------------------------------------------------
	 ; Check If launched rom is part of a set so we know to cancel GUI or not. Then check If 7z support is needed.
	;-----------------------------------------------------------------------------------------------------------------------------------------
	If !mgCandidate
	{	RLLog.Warning("StartMulti - User requested MG, but """ . dbName . """ is not a candidate for MultiGame.")
		Return
	}
	...
	If (pauseEnabled = "true")
		XHotKeywrapper(pauseKey,"TogglePauseMenuStatus","OFF")	; turning off the Pause key so nobody can bring up Pause while MG is active, it will crash or cause advers affects
	...
	If (!romTable.MaxIndex() ) {
		...
		RLLog.Warning("StartMulti - This game does not need Multi-Game support or your rom names are not named correctly so that RocketLauncher can link them together.")
		Sleep, 1000
		ToolTip
		XHotKeywrapper(mgKey,"StartMulti","ON")	; turning back on mgKey
		If (pauseEnabled = "true")
			XHotKeywrapper(pauseKey,"TogglePauseMenuStatus","ON"), On	; turning back on pauseKey
		XHotKeywrapper(exitEmulatorKey,"CloseProcess","ON")
		RLLog.Debug("StartMulti - Ended")
		Return	; required for RocketLauncher, this will return back to the emu If rom does not qualify for MG
	}
	...
	RLLog.Debug("StartMulti - Your current screen's resolution is " . A_ScreenWidth . "x" . A_ScreenHeight)
	RLLog.Debug("StartMulti - Your emulator """ . mgEmuProcessName . """ is running at " . emuW . "x" . emuH)
	...
	RLLog.Debug("StartMulti - Halting emu If module contains a HaltEmu label.")
	Gosub, HaltEmu
	RLLog.Debug("StartMulti - Finished Processing HaltEmu label.")
	...
	If rlMode
		disableSuspendEmu := true	; must disable this otherwise launching mg directly suspends RocketLauncher

	If !disableSuspendEmu { 
        ProcSus(mgEmuProcessName)
        RLLog.Debug("Emulator process suspended")
    }
	
	If (keymapperEnabled = "true") and (keymapperRocketLauncherProfileEnabled = "true") and (keymapper != "ahk")
		RunKeyMapper%zz%("menu",keymapper)	; If user desires, load RocketLauncher profile for controllers
    ...
    If FileExist(RLMediaPath . "\Backgrounds\" . systemName . "\"  . dbName . "\*.*")
        Loop, parse, Supported_Images,`,,
            Loop, % RLMediaPath . "\Backgrounds\" . systemName . "\"  . dbName . "\*." . A_LoopField
                MGBackground.Insert(A_LoopFileFullPath)
    If !MGBackground[1]
        If FileExist(RLMediaPath . "\Backgrounds\" . systemName . "\"  . DescriptionNameWithoutDisc . "\*.*")
            Loop, parse, Supported_Images,`,,
                Loop, % RLMediaPath . "\Backgrounds\" . systemName . "\"  . DescriptionNameWithoutDisc . "\*." . A_LoopField
                    MGBackground.Insert(A_LoopFileFullPath)
    If !MGBackground[1]
    {
    ...
    If !MGBackground[1]
        If FileExist(RLMediaPath . "\Backgrounds\" . systemName . "\_Default\*.*")
            Loop, parse, Supported_Images,`,,
                Loop, % RLMediaPath . "\Backgrounds\" . systemName . "\_Default\*." . A_LoopField
                    MGBackground.Insert(A_LoopFileFullPath)
    If !MGBackground[1]
    {
        for index, element in feMedia["Backgrounds"]
    ...
    If !MGBackground[1]
        If FileExist(RLMediaPath . "\Backgrounds\" . "_Default\*.*")
            Loop, parse, Supported_Images,`,,
                Loop, % RLMediaPath . "\Backgrounds\" . "_Default\*." . A_LoopField, 0
                    MGBackground.Insert(A_LoopFileFullPath)
	If MGBackground[1] {
	 	RLLog.Debug("MultiGame - Randomizing found background images.")
        Random, RndmBackground, 1, % MGBackground.MaxIndex()
        multiBG := MGBackground[RndmBackground]
		RLLog.Debug("MultiGame - Using background art: " . multiBG)
    }

	; Search for Default Artwork used for media types
	Loop, 2 {	; Load the 2 files into variables for easy calling later
		mgDefaultArtPath1 := RLMediaPath . "\MultiGame\" . systemName . "\_Default\" . romTable[A_Index,6] . "_image_" . A_Index . ".png"
		mgDefaultArtPath2 := RLMediaPath . "\MultiGame\_Default\" . romTable[A_Index,6] . "_image_" . A_Index . ".png"
		mgDefaultArtIndex := A_Index	; so it can be used in the next loop
		mgArtSizeCheck := ""
		Loop, 2 {
			RLLog.Debug("MultiGame - Looking for Default " . romTable[A_Index,6] . " Art in: " . mgDefaultArtPath%A_Index%)
			If FileExist(mgDefaultArtPath%A_Index%) {
				Image_%mgDefaultArtIndex% := mgDefaultArtPath%A_Index%
				RLLog.Debug("MultiGame - Found Default " . romTable[A_Index,6] . " Art: " . mgDefaultArtPath%A_Index%)
				foundDefaultMGArt := 1
				Break
			}
		}
		If !foundDefaultMGArt {
			RLLog.Debug("MultiGame - Could not locate any Default " . romTable[A_Index,6] . " Art, downloading default " . romTable[A_Index,6] . " artwork to: " . mgDefaultArtPath)
			ToolTip, Downloading Image %A_Index%,0,0
			IfNotExist, %  RLMediaPath . "\MultiGame\_Default\"
				FileCreateDir, %  RLMediaPath . "\MultiGame\_Default\" ; Need to create the folder first otherwise urldownload will fail
			...
			If (mgArtSizeCheck <= 1)	; if the download failed, it could possibly be an html 404 msg as the png file which is usually less then 1KB.
				ScriptError("Error connecting to GIT to download default MultiGame images. Please try again later and report the problem If it persists or copy default images to: " . RLMediaPath . "\MultiGame\_Default\")
		}
	...
	RLLog.Debug("MultiGame screen scale factor: X=" . mgXScale . ", Y= " . mgYScale)
	...
	for index, element in romTable	; for each rom found in the table
	{
		...
		{	mgGameArtPath1 := RLMediaPath . "\MultiGame\" . systemName . "\" . dbName . "\" . romTable[A_Index,6] . "_image_" . A_Index . ".png"
			mgGameArtPath2 := RLMediaPath . "\MultiGame\" . systemName . "\" . DescriptionNameWithoutDisc . "\" . romTable[A_Index,6] . "_image_" . A_Index . ".png"
			mgGameArtPath3 := feDiscChangeDir . "\" . romTable[A_Index, 3] . ".png"
			mgGameArtIndex := A_Index	; so it can be used in the next loop
			Loop, 3 {
				RLLog.Debug("MultiGame - Checking for Game " . romTable[mgGameArtIndex,6] . " Art: " . mgGameArtPath%A_Index%)
				If FileExist(mgGameArtPath%A_Index%) {
					RLLog.Debug("MultiGame - Using Game Art for " . romTable[mgGameArtIndex,6] . " " . mgGameArtIndex . ": " . mgGameArtPath%A_Index%)
		...
		If !foundGameArt {
			RLLog.Debug("MultiGame - " . (mgUseGameArt = "true" ? "No Game Art found" : "Game Art disabled") . ". for " . romTable[A_Index,6] . " " . A_Index . ", using default: """ . Image_1 . """ and """ . Image_2 . """")
...
SelectGame:
	...
	RLLog.Info("SelectGame - User selected to load: " . selectedRom)
	...
	If mgLaunchMenuActive {
		RLLog.Info("SelectGame - Updating romPath and romExtension because user chose a new game in the MG launch menu.")
		romPath := mgRomPath
		romExtension := mgRomExt
		dbName := mgDbName
		; exitapp
	} Else If (sevenZEnabled = "true")	; Only need to continue If 7z support is turned on, this check is in case emu supports loading of compressed roms. No need to decompress our rom If it does
	{
		If StringUtils.Contains(mgRomExt,sevenZFormats)	; Check If our selected rom is compressed.
		{
			RLLog.Info("SelectGame - This game needs 7z to load. Sending it off for extraction: " . mgRomPath . "\" . mgDbName . mgRomExt)
			sevenZ%currentButton% := 7z(mgRomPath, mgDbName, mgRomExt, sevenZExtractPath, "mg")	; Send chosen game to 7z for processing. We get back the same vars but updated to the new location.
			selectedRom := mgRomPath . "\" . mgDbName . mgRomExt
			RLLog.Info("SelectGame - Returned from 7z extraction, path to new rom is: " . selectedRom)
			romTable[currentButton,19] := mgRomPath	; storing path to extracted rom in column 19 so 7zCleanUp knows to delete it later
			RLLog.Debug("SelectGame - Stored """ . mgRomPath . """ for deletion in 7zCleanup.")
		} Else
			RLLog.Info("SelectGame - This game does not need 7z. Sending it directly to the emu or to Virtual Drive If required.")
	}
	mgSelectedGame := 1	; filling var so we know user selected a game
	RLLog.Info("SelectGame - Ended")
	Goto, MGExit
Return
...
MGProgressBarAnimation:
	...
	RLLog.Info("MGProgressBarAnimation - Started")
	...
	Loop {
		...
		Else {	; bar is at 100%
			mgFinishedBar := 1
			RLLog.Debug("MGProgressBarAnimation - Bar reached 100%")
			If (fadeBarInfoText = "true")
		...
		{	RLLog.Debug("MGProgressBarAnimation - 7z.exe is no longer running, breaking out of progress loop.")
			Break
		}
		If mgFinishedBar
			Break
	...
	RLLog.Info("MGProgressBarAnimation - Ended")
Return

MGCancel:
	mgCancel := 1
MGExit:
	RLLog.Info("MGExit - Started")
	SetTimer,Update, Off
	ToolTip
	...
	{	ProcRes(mgEmuProcessName)
		RLLog.Info("MGExit - Emulator process started")
	}
	...
	{	timeout := A_TickCount
		...
		{	Loop
			...
			RLLog.Info("Emulator screen reactivated")
		}
	}
	RLLog.Info("MGExit - Restoring emu If module contains a RestoreEmu label.")
	Gosub, RestoreEmu
	RLLog.Info("MGExit - Finished Processing RestoreEmu label.")
	...
	RLLog.Info("MGExit - Black Screen Gui destroyed")
	XHotKeywrapper(exitEmulatorKey,"CloseProcess","ON")
	XHotKeywrapper(mgKey,"StartMulti","ON")	; turning back on mgKey
	If (pauseEnabled = "true")
		XHotKeywrapper(pauseKey,"TogglePauseMenuStatus","ON"), On	; turning back on pauseKey
	...
	{	Gosub, EnableBezelKeys	; turning on the bezel keys
   	...
   	RLLog.Info("MGExit - Enabled Exit Emulator, Bezel, Pause and MultiGame keys")
	If (keymapperEnabled = "true") and (keymapperRocketLauncherProfileEnabled = "true") and (keymapper != "ahk") {
		RLLog.Info("MGExit - Running keymapper to load the proper profile.")
	...
	If (disableActivateBlackScreen != "true")	; set this to true in HaltEmu label to disable this GUI
		Gui, 19: Destroy
	IfWinNotActive, ahk_class %mgEmuClass%,,ahk_pid %focusAppOnExit% 	; focusAppOnExit is from RocketLauncher, the PID is from the FE the user is using. We use this here as to be sure we never give it focus.
		Loop{ 
	...
	If mgSelectedGame && !mgLaunchMenuActive {
		...
		{	RLLog.Info("MGExit - Updating Statistics.")
			Gosub, UpdateStatistics
		}
        gameSectionStartTime := A_TickCount
		gameSectionStartHour := A_Now
		RLLog.Info("MGExit - Processing MultiGame label in module.")
		Gosub, MultiGame
		RLLog.Info("MGExit - Finished Processing MultiGame label.")
	}
	If !forceMGGuiDestroy {
		Loop, 2 {
			Gdip_GraphicsClear(mgG%A_Index%)
	...
	If (mgLaunchMenuActive && mgCancel) {	; user canceled the MG Launch menu
		RLLog.Warning("MGExit - User canceled the MultiGame Launch menu. Exiting RocketLauncher.")
		Exitapp
	}
