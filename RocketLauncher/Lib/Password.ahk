...
InputPasswordMenu() {
	Global
	RLLog.Info("InputPasswordMenu - Started")
	...
	PasswordBitmap := Gdip_CreateBitmapFromFile(RLMediaPath . "\Menu Images\RocketLauncher\Password.png")
	...
	Loop
		If inputPasswordMenuExit
			Break
	RLLog.Info("InputPasswordMenu - Ended")
	Return
}

DestroyInputPasswordMenu() {
	Global
	RLLog.Info("DestroyInputPasswordMenu - Started")
	...
	RLLog.Info("DestroyInputPasswordMenu - Ended")
	Return
}
...
PassSelect:
	If (launchPasswordCheck = 1) {
		RLLog.Info("User entered the correct password.")
		...
		RLLog.Info("User entered an incorrect password.")
		UserInput := ""
		maskedUserInput := ""
...
PassExitMenu:
	RLLog.Info("User canceled out the Input Password Menu.")
	DestroyInputPasswordMenu()
	ExitModule()
Return
...
PassP2Right:
	...
	launchPasswordCheck := RLObject.VerifyPassword(UserInput, launchPasswordHash)
	If (launchPasswordCheck = 1)
		Gosub, passSelect
	Else If (passTotalLength >= 16) {
		If rocketLauncherIsExiting	; prevents very fast inputs from triggering ExitModule() more than once
			Return
		RLLog.Error("The password entered exceeded the maximum length and was incorrect. Returning to your Front End.")
		ExitModule()
	}
	; Tooltip, % UserInput
Return
