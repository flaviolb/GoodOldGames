...
{	Log("Keymapper - Checking for the JoyIDs_Preferred_Controllers key  in " . A_ScriptDir . "\Settings\" . systemName . "\Games JoyIDs.ini under section [" . dbName . "]", 5)
	IniRead, JoyIDsPreferredControllersRom, %A_ScriptDir%\Settings\%systemName%\Games JoyIDs.ini, %dbName%, JoyIDs_Preferred_Controllers ;checking to see if user has per game settings RocketLauncherUI may or may not want to support it but being able to do it per game is important.
	;Converting from rom -> emu -> system -> global to get the correct Preferred controller list.
	JoyIDsPreferredControllersSystem := If (!JoyIDsPreferredControllersRom || JoyIDsPreferredControllersRom != "ERROR") ? JoyIDsPreferredControllersRom : JoyIDsPreferredControllersSystem
	; MsgBox JoyIDsPreferredControllersGlobal: %JoyIDsPreferredControllersGlobal%`nJoyIDsPreferredControllersSystem: %JoyIDsPreferredControllersSystem%`nJoyIDsPreferredControllersRom: %JoyIDsPreferredControllersRom%`n
	If !JoyIDsPreferredControllersSystem
		JoyIDsPreferredControllersSystem := "use_global"
	JoyIDsPreferredControllers := (If JoyIDsPreferredControllersSystem = "use_global" ? 1 : JoyIDsPreferredControllersSystem)
	If JoyIDsPreferredControllers
		LoadPreferredControllers(JoyIDsPreferredControllers)
	Else
		ScriptError("Error: You supplied an empty Preferred Controller List.`nSettings for this can be found in`n" . emuFile . "`n" . systemName . "\RocketLauncher.ini`nGlobal RocketLauncher.ini`n`nPlease make sure to include the controller name in this format:`nController Name 1|Controller Name 2|Controller Name 3 . (the numbers are not needed.)",,,300)
}
...
{	If (keymapper = "xpadder")
	...
	Else If (keymapper = "joytokey" || keymapper = "joy2key") {
		...
		ScriptError("Keymapper - Invalid keymapper method: " . keymapper)
	Log("Keymapper - Loading " . keymapper)
	keymapperLoaded := 1	; if an error occurs before this is loaded, will not try to unload keymapper on exit
