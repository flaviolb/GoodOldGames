...
MSystem := ["American Laser Games","WoW Action Max","Daphne","LaserDisc"]
;----------------------------------------------------------------------------
; Notes:
...
; For example,  If your rompath is C:\Roms\American Laser Games\, the maddog game would be found in C:\Roms\American Laser Games\maddog\
; and the framefile would be in C:\Roms\American Laser Games\maddog\maddog.txt
...
; ("singe/
; New:
; ("//SERVER/Roms/American Laser Games/
;
; If using a local drive, it would look something like this "C:/Roms/American Laser Games/"

; WoW Action Max
; Emu_Path should be something like this C:\Emulators\WoW Action Max\daphne-singe\
...
; "singe/Action Max/
; New:
; "//SERVER/Roms/WoW Action Max/
;
; If using a local drive, it would look something like this C:/Roms/WoW Action Max/
;
...
singePathUpdate := IniReadCheck(settingsFile, "settings", "SingePathUpdate","false",,1)
...
SingePathUpdate(){
	...
	If InStr(romSingeData, rompathCommonSlash)
		If (forcePathUpdate = "false") {
			Log("SingePathUpdate - """ . romSingeFile . """ contains the current Rom Path. Skipping the SingePathUpdate function because your files appear to have the correct paths already set. If you want to force a path update, please set the ForcePathUpdate variable in the Daphne Singe module options in RocketLauncherUI to true.")
	...
	Log(updatedLines . " Singe paths updated by RocketLauncher.")
	Log("SingePathUpdate - Ended")
	exitapp
	Return
}
