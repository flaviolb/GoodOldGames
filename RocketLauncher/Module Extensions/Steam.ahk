...
Steam(stmID:="", stmProtocol:="", params:="") {
	...
	SteamLaunch:	; steam is not running
		...
		sP := Decrypt(sP,"stk")
		If (!sU || !sP)
			ScriptError("SteamLaunch - Steam is not running and needs to be logged in to launch this steam game. RocketLauncher can do this, but you need to run ""EncryptPasswords"" application in your PCLauncher module folder first and set your login credentials.")
...
GetSteamPath() {
	...
	If !FileExist(steamFullPath) {
		Log("GetSteamPath - Ended - Steam not found at: " . steamFullPath)
		Return
	} Else {
