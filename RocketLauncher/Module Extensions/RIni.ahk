...
RIni_Read(RVar, File, Correct_Errors=1, Remove_Inline_Key_Comments=0, Remove_Lone_Line_Comments=0, Remove_Inline_Section_Comments=0, Treat_Duplicate_Sections=1, Treat_Duplicate_Keys=1, Read_Blank_Sections=1, Read_Blank_Keys=1, Trim_Spaces_From_Values=0, ByRef RIni_Read_Var = "")
{
	...
	Loop, Parse, RIni_Read_Var, `n, `r
	{
		...
		If Has_Equal
		{
			...
			If Section_Skip
				Continue
			TKey := Trim(SubStr(A_LoopField, 1, Has_Equal-1))
...
RIni_GetRLKeyValue(RVar, Sec, Key)
{
	Return RIni_GetKeyValue(RVar, Sec, Key, "ERROR_INVALID_REFERENCE", "ERROR_SECTION_NOT_FOUND", "ERROR_KEY_NOT_FOUND")
}

RIni_GetKeyValue(RVar, Sec, Key, Default_Invalid_Ref_Return="", Default_No_Section_Return="", Default_No_Key_Return="")
{
	Global
	
	If (!RIni_%RVar%_Is_Set)
		Return Default_Invalid_Ref_Return = "" ? -10 : Default_Invalid_Ref_Return
	Sec := RIni_CalcMD5(Sec)
	
	If !RIni_%RVar%_%Sec%_Is_Set
		Return Default_No_Section_Return = "" ? -2 : Default_No_Section_Return
	Key := RIni_CalcMD5(Key)
	
	If (%RVar%_All_%Sec%_Keys){
		If (!InStr("`n" %RVar%_All_%Sec%_Keys, "`n" Key "`n"))
			Return Default_No_Key_Return = "" ? -3 : Default_No_Key_Return
		If (%RVar%_%Sec%_%Key%_Value != "")
			Return %RVar%_%Sec%_%Key%_Value
		Else
			Return Default_No_Key_Return
	} Else
		Return Default_No_Key_Return = "" ? -3 : Default_No_Key_Return
}
